...
public class PentominoDatabase {
	 ...
	 * @throws FileNotFoundException if the file is not found (probably, wrong location of the file and/or typo in the fileName parameter)
	 */
	private static int[][][][] loadData(final String fileName) {
		...
		try {
			...
			while (scanner.hasNextLine()) // For each line in the CSV file
			{
				...
				final int xSize = Integer.parseInt(values[2]);
				final int ySize = Integer.parseInt(values[3]);
				final int[][] piece = new int[xSize][ySize];
