...
public class PentominoBuilder {
	...
	public static void makeDatabase() {
		...
		for (final int[][] ints : basicDatabase) {
	...
	public static int[][] makeBigger(final int[][] data, final int size) {
		...
		final int[][] returnData = new int[size][size];
	 ...
	 */
	public static int[][] moveToAbove(final int[][] data) {
		...
		for (int i = 0; i < data[0].length; i++) {
			...
			if (empty == 0) {
				...
				for (final int[] datum : data) {
					...
					//data[j][k - 1] = data[j][k];
					if (datum.length - 1 >= 0) System.arraycopy(datum, 1, datum, 0, datum.length - 1);
				}
				amountToCut++;
			}
		}
	 ...
	 */
	public static int[][][] eraseDuplicates(final int[][][] data) {
		...
		final int[][][] returnData = new int[counter][][];
	 ...
	 */
	public static int[][] eraseEmptySpace(final int[][] data) {
		...
		final int[][] returnData = new int[amountOfColumns][amountOfRows];
	...
	public static void main(final String[] args) throws IOException {
		...
		final PrintWriter writer = new PrintWriter("pentominos.csv", StandardCharsets.UTF_8);
